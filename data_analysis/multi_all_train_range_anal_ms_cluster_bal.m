function output_struct=multi_all_train_range_anal_ms_cluster_bal(stringa_dir_batch,par,stringa_name)
...
fid = fopen([stringa_dir_batch stringa_name '_All' '_nan_count.txt'],'w');
...
if PLOT.print
    ...
    export_fig(gcf,[figure_name stringa_name '_bal_' names_string '.png']);
end
...
set(gca,'XTickLabels',strrep(field_names(outperm),'_','\_'),'FontSize',7);
xtickangle(gca,45);
if PLOT.print
...
set(gca,'XTickLabels',strrep(field_names_wc_ms(outperm),'_','\_'),'FontSize',7);
xtickangle(gca,45);
if PLOT.print
...
set(gca,'XTickLabels',strrep(field_names(outperm),'_','\_'),'fontsize',7); % this sets the scale for all axis text properties... ok for now
xtickangle(gca,45);
...
set(gca,'XTickLabels',strrep(field_names_wc_ms(outperm),'_','\_'),'fontsize',7); % this sets the scale for all axis text properties... ok for now
xtickangle(gca,45);
...
if PLOT.print
    ...
    export_fig(gcf,[figure_name stringa_name '_bal_' names_string '.png'],'-m3');
end
...
not_shown={'modulus_m','modulus_mn','emd','emdn','bursty','PPC','schreiber_c','vRn','VPN','HM','vR','VP','cv_ISI','Lv_ISI','LCV_ISI','IR_ISI','SM_ISI','QQA','kruskal_c','sttc','fooof_1p_offset','psd_max'};
add_ms;
...
set(gcf,'Position',[1000         392        1476         942]);
set(gca,'XTickLabels',strrep(field_names(outperm_spearman),'_','\_'),'FontSize',7);
xtickangle(gca,45);
if PLOT.print
...
[H,T,outperm_spearman] = dendrogram(sync_all_link_absspearmancorr_avg,0);
set(gcf,'Position',[1000         392        1476         942]);
set(gca,'XTickLabels',strrep(field_names_wc_ms(outperm_spearman),'_','\_'),'FontSize',7);
xtickangle(gca,45);
if PLOT.print
...
set(gcf,'Position',[1000         392        1476         942]);
set(gca,'XTickLabels',strrep(field_names_long_wc_ms(outperm_spearman),'_','\_'),'FontSize',7);
xtickangle(gca,45);
if PLOT.print
    figure_name='sync_all_absspearmancorr_avg_dendro_color_longn';
    export_fig(gcf,[figure_name stringa_name '_bal_' names_string '.png'],'-m2');
end
...
set(gca,'xaxisLocation','top')
set(gca,'XTick',1:1:length(field_names));
set(gca,'XTickLabels',strrep(field_names(outperm_spearman),'_','\_'),'fontsize',7); % this sets the scale for all axis text properties... ok for now
xtickangle(gca,45);
set(gca,'YTick',1:1:length(field_names));
...
set(gca,'xaxisLocation','top')
set(gca,'XTick',1:1:length(field_names_wc_ms));
set(gca,'XTickLabels',strrep(field_names_wc_ms(outperm_spearman),'_','\_'),'fontsize',7); % this sets the scale for all axis text properties... ok for now
xtickangle(gca,45);
set(gca,'YTick',1:1:length(field_names_wc_ms));
...
set(gca,'xaxisLocation','top')
set(gca,'XTick',1:1:length(field_names));
set(gca,'XTickLabels',strrep(field_names_long_wc_ms(outperm_spearman),'_','\_'),'fontsize',4); % this sets the scale for all axis text properties... ok for now
xtickangle(gca,45);
set(gca,'YTick',1:1:length(field_names));
set(gca,'YTickLabels',strrep(field_names_long_wc_ms(outperm_spearman),'_','\_'),'fontsize',4); % this sets the scale for all axis text properties... ok for now
cbh=colorbar;
pos_vect=get(cbh,'Position');
th2=annotation('textbox',[pos_vect(1)+pos_vect(3)+0.01 pos_vect(2)+0.46*pos_vect(4) 0.012 0.046],'String','|\rho|','FontSize',24,'LineStyle','none','VerticalAlignment','middle');
if PLOT.print
...
sa=axes('Position',[pos_tight(1)    pos_tight(2)+pos_vect(4)+0.005    pos_tight(3)    0.0552]);
for iSYNC=1:NSYNC(1)
    iSYNC_perm=outperm_spearman(iSYNC);
    text(iSYNC,0.5,strrep(field_names_long_wc_ms(iSYNC_perm),'_','\_'),'HorizontalAlignment','center','VerticalAlignment','middle','Rotation',90,'FontSize',4);
    hold on;
end
...
clear output_this;
whos
save(['cluster' stringa_name '_bal_output' '_' names_string]);
% keyboard;
output_struct=[];
cd ..
