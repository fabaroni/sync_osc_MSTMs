function output_struct=analyze_osc_train_fooof_win_data(dataDir,filenames,stringa,par)
    ...
    if n_neu_this<n_neu_min
        ...
        for i_sample=1:n_sample
            ...
            for iwin=1:nwin
                ...
                output_this=analyze_osc_train_fooof_fun_plot(spiketimes_win,par_this,field_names); % in case we wanted to generate figures - not recommended for every win and every recording, but we can control plotting with par.PLOT
                for kk=1:length(field_names)
                    try
            ...
            save(fullfile(dataDir,strrep(filenames{ifile},'.mat',['_measures_fooof_sofar.mat'])),'output_measures','neu_ind_mat','par','par_this'); % better to save for each sample
        end
    end
    % keyboard;
    save(fullfile(dataDir,strrep(filenames{ifile},'.mat','_measures_fooof.mat')),'output_measures','neu_ind_mat','par','par_this');
end
