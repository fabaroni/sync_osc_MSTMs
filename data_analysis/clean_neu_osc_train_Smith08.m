function output_struct=clean_neu_osc_train_Smith08(dataDir,cleandataDir,filenames,stringa,par)
...
for ifile=1:nfiles
    clear spiketimes spiketimes_clean n_spikes t_first t_last min_isi n_short_isi percent_short_isi n_consecutive_short_isi clean_neu_vect;
    ...
    for i_neu=1:n_neu_this
        ...
        if percent_short_isi(i_neu) < percent_short_isi_thr
    ...
    save(fullfile(cleandataDir,filenames{ifile}),'spiketimes_clean','par');
...
xlabel('# neu removed');
ylabel('count');
title(['rm neu w more than ' num2str(percent_short_isi_thr*100) '% of ISI<1ms '],'FontWeight','normal','Interpreter','none');
