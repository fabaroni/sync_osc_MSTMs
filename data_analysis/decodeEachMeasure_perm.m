...
clear nan_ind
if isfield(par,'seed')
    rng(par.seed); % Better initializing with rng rather than rand % https://www.mathworks.com/help/matlab/ref/rng.html
end
...
switch Job.decodeMode
    ...
    case 'pair'
        for jfn=(par.ifn_this+1):length(field_names)

            %%
            for ifile=1:nfiles
                clear tmp2
                for iClass = 1:nClass
                    cResp{ifile,iClass} = [tmp{ifile,iClass}.([field_names{par.ifn_this} '_all'])' tmp{ifile,iClass}.([field_names{jfn} '_all'])'];

                    nan_ind{ifile,iClass}(jfn,:,:)=isnan(cResp{ifile,iClass});
                    if any(any(nan_ind{ifile,iClass}(jfn,:,:)))
                        nan_ind_this=squeeze(any(nan_ind{ifile,iClass}(jfn,:,:),3));
            ...
            zmcorrTrain.([field_names{par.ifn_this} '__' field_names{jfn}]) = P.zmcorrTrain;
            zmcorrTest.([field_names{par.ifn_this} '__' field_names{jfn}]) = P.zmcorrTest;
            stdcorrTrain.([field_names{par.ifn_this} '__' field_names{jfn}]) = P.stdcorrTrain;
            stdcorrTest.([field_names{par.ifn_this} '__' field_names{jfn}]) = P.stdcorrTest;

        end
        decodeResultFilename = ['pair_' ...
            Job.decodeType '_loro_' num2str(par.ifn_this) '_norm.mat' ];
end
%%
whos
