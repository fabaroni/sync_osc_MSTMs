...
data_file=char(data_file); % more portable with char, also works for cell arrays
data_file=strrep(data_file,'.mat','');
...
for iss=1:length(ss_names)
    if isfield(ss_data,ss_names(iss))
        eval([char(ss_names(iss)) '=ss_data.' char(ss_names(iss)) '(1:min(nwin,nwin_ss));']); % we just retain the first min(nwin,nwin_ss)
    else
        keyboard;
    end
end
...
for ifn=1:length(field_names)
    ...
    data_now.([field_names{ifn} '_all'])=data_this(1:min(nwin,nwin_ss));
end
...
switch Job.decodeType
    case {'wake_vs_nrem',... % 1
        ...
        for ifn=1:length(field_names)
            data_this = data_now.([field_names{ifn} '_all']);
            eval(['data_this_1 = data_this(logical(' c1_string '_3ormore_vect));']);
            eval(['data_this_2 = data_this(logical(' c2_string '_3ormore_vect));']);
            tmp{1}.([field_names{ifn} '_all'])=data_this_1;
            tmp{2}.([field_names{ifn} '_all'])=data_this_2;
        end
        for iClass = 1:nClass
            nTrialsPerClass(iClass) = length(tmp{iClass}.([field_names{ifn} '_all']));
        end
...
min_n_samples=min(nTrialsPerClass);
clear P R CC
clear nan_ind
...
switch Job.decodeMode
    case 'single'
        for ifn=1:length(field_names)
            ...
            clear tmp2
            for iClass = 1:nClass
                cResp{iClass} = tmp{iClass}.([field_names{ifn} '_all'])';

                nan_ind{iClass}(ifn,:)=isnan(cResp{iClass});
                if any(nan_ind{iClass}(ifn,:))
                    cResp{iClass} = cResp{iClass}(~nan_ind{iClass}(ifn,:)); % remove NaNs
                end
                min_n_samples=min(min_n_samples,length(cResp{iClass}));
            end

            % Pattern normalization... not here anymore, but inside evaluatePerformance_mem.m
            [ind_train,ind_test,perm_class_ind] = leaveOneOutRLSC_perm(cResp,C);
            [P] = evaluatePerformance_perm(cResp,C,ind_train,ind_test,perm_class_ind);
    ...
    case 'pair'
        for ifn=1:length(field_names)
            ...
            for jfn=(ifn+1):length(field_names)
                ...
                clear tmp2
                for iClass = 1:nClass
                    cResp{iClass} = [tmp{iClass}.([field_names{ifn} '_all'])' tmp{iClass}.([field_names{jfn} '_all'])'];

                    nan_ind{iClass}(ifn,jfn,:,:)=isnan(cResp{iClass});
                    if any(any(nan_ind{iClass}(ifn,jfn,:,:)))
                        nan_ind_this=squeeze(any(nan_ind{iClass}(ifn,jfn,:,:),4));
                        cResp{iClass} = cResp{iClass}(~nan_ind_this,:); % remove rows with at least one NaNs
                    end
                    min_n_samples=min(min_n_samples,length(cResp{iClass}));
                end
