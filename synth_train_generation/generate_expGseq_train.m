function output_struct=generate_expGseq_train(stringa,par)
...
% par_string={'t_refr','n_neu','rate','r_osc_ampl','f_osc','transient','sim_time','seed','inct','Rp_dt','sigma_omega','tau_OUnoise','p_fail','sigmaGpercent','duty_cycle'};
par_string={'t_refr','n_neu','rate','r_osc_ampl','f_osc','transient','sim_time','seed','inct','Rp_dt','tau_OUnoise','p_fail','sigmaGpercent','duty_cycle'};
...
while (time<transient)
    if time >= n_event
        ...
        for i=1:n_neu
            if ~fail_neu(i)
                spike_this=pop_spike+seq_t_vect(i)+rel_spiketimes(i)-transient;
