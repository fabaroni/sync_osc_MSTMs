...
#define N_ST_I prhs[0] // number of spikes in spike train i
#define N_ST_J prhs[1] // number of spikes in spike train j
#define ST_I prhs[2] // spike train i
#define ST_J prhs[3] // spike train j
#define A prhs[4] // start time
#define B prhs[5] // end time


  int *n_st_i, *n_st_j,i;
  double *a, *b,*inct,*time,*index,*sttc,*st_i,*st_j;
  ...
  // SPIKE_TRAIN strain_i(n_st_i); // compilation error
  // SPIKE_TRAIN strain_j(n_st_j);
  // SPIKE_TRAIN strain_i((size_t) n_st_i); // What() is:std::bad_alloc
  // SPIKE_TRAIN strain_j((size_t) n_st_j);
  SPIKE_TRAIN strain_i((size_t) *n_st_i); // this works
  SPIKE_TRAIN strain_j((size_t) *n_st_j);
  // SPIKE_TRAIN strain_i; // matlab crashes
  // SPIKE_TRAIN strain_j;
  ...
  //  index=do_measure_optimal(strain_i,strain_j,a,b); // doesn't work: cannot convert ‘double*’ to ‘double’ for argument ‘3’ to ‘double do_measure_optimal(SPIKE_TRAIN, SPIKE_TRAIN, double, double)’
  //  *index=do_measure_optimal(strain_i,strain_j,*a,*b); // doesn't work: error: cannot convert ‘double’ to ‘double*’ in assignment
  *index=do_measure_optimal(strain_i,strain_j,*a,*b); // works
