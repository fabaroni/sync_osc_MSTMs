...
for n=1:nw;
   ...
   datawin=data(indx,:);
   [datafitwin,as,fs]=fitlinesc(detrend(datawin),params,p,'n',f0);
   % [datafitwin,as,fs]=fitlinesc(detrend(datawin),params,p,'y',f0); % for debug
   % [datafitwin,as,fs]=fitlinesc(datawin,params,p,'n',f0);
   Amps{n}=as;
   freqs{n}=fs;
   datafitwinthis=datafitwin;
