function output_struct=multi_train_range_runall(stringa,stringa_mfile_osc,stringa_mfile_ple,par,ranged_par)
% organises simulations (ranging rate f_osc and r_osc_ampl) and m files to be submitted to a PBS queue
par_this=par;
...
script_name=[stringa '_' suf1  '_' suf2 '_matched_runall.m'];
fid = fopen(script_name,'w');
for i=rate_start:rate_stop
    for j=f_osc_start:f_osc_stop
        for k=r_osc_ampl_start:r_osc_ampl_stop
            comm=['script_' stringa '_' num2str(i) '_' num2str(j) '_' num2str(k) '_' suf1];
            fprintf(fid,[comm '\n']);
            fprintf(fid,['clear all;close all\n']);
            
            comm=['script_' stringa '_' num2str(i) '_' num2str(j) '_' num2str(k) '_' suf2];
            fprintf(fid,[comm '\n']);
            fprintf(fid,['clear all;close all\n']);
        end
    end
...
fprintf(['nohup matlab -nodesktop -noopengl < ' script_name ' > output_file_' strrep(script_name,'.m','') '\n']);